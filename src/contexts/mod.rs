...
pub trait BuildContext {
    ...
    fn log(&self) {
        color_log(Color::Yellow, &format!("Building {}", self.context_name()));
    }
    ...
    fn is_file_in_context(&self, absolute_file_path: &Path) -> bool {
        ...
        self.get_entrypoints().iter().any(|entry| {
            ...
            let absolute_folder_path = folder_path
                .canonicalize()
                .unwrap_or_else(|_| std::env::current_dir().unwrap().join(folder_path));
